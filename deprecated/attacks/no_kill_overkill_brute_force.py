		...
		if following_codon.start == codon.start + codon.length:
			if ((not codon.status.isdisjoint(outgoing)) and (not following_codon.status.isdisjoint(incoming))) or ((not codon.status.isdisjoint(blunt_out)) and (not following_codon.status.isdisjoint(blunt_in))):
				...
				if status != set("unverified"):
					...
					results.write(str(codon.length + following_codon.length) + "\t" + codon.word + following_codon.word + "\t" + str(codon.start) + "\t" + codon.expression + following_codon.expression + "\t" + str(status) + "\n")
results.close()
		...
		if following_codon.start == codon.start + codon.length:
			if ((not codon.status.isdisjoint(outgoing)) and (not following_codon.status.isdisjoint(incoming))) or ((not codon.status.isdisjoint(blunt_out)) and (not following_codon.status.isdisjoint(blunt_in))):
				...
				if status != set("unverified"):
					...
					results.write(str(codon.length + following_codon.length) + "\t" + codon.word + following_codon.word + "\t" + str(codon.start) + "\t" + codon.expression + following_codon.expression + "\t" + str(status) + "\t" + str(round(evaluate(codon.expression + following_codon.expression, frequencies), 2)) + "\n")
results.close()
