...
# Open output file.
filename = "double_autokey"
results = open(filename + "_" + dictionaryName + ".output", 'w', 1)
results.write("Length 1\tLength 2\tTotal Length\tKey 1\tKey 2\tPlaintext\n")

# Set logging.
attemptsPerMessage = 1000
attempts = 0
		...
		for key2 in sorted(words):
			if len(key2) > minKeyLength and not key1 > key2:
				...
				if not validityText(dictionary, {}, plaintext).isdisjoint({"beginnings", "words", "words_beginnings"}):
					results.write(str(len(key1)) + "\t" + str(len(key2)) + "\t" + str(len(key1) + len(key2)) + "\t" + key1 + "\t" + key2 + "\t" + plaintext + "\n")
				attempts += 1
