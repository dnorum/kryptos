...
# Open output file.
filename = "key_autokey_ends_" + dictionaryName
...
for word1 in keyDictionary1:
	...
	# Resuming...
	if word1.strip().upper() < "THICKNESS":
		continue
		...
		for i in range(0, len(word)):
			...
			if length < len(key) and length < min(len(segment1), len(segment2)):
				for keySegment in keySubstrings[length]:
					keystreamFragment = QuagmireIII(alphabet, "KRYPTOS", keySegment, "K").encrypt(end)
					if (keystreamFragment == segment1[-length:]) or (keystreamFragment == segment2[-length:]):
						hit = str(length) + "\t" + word1.strip().upper() + "\t" + keySegment + "\t" + end + "\t" + keystreamFragment + "\n"
						if hit not in hits:
							results.write(hit)
							hits.add(hit)
				attempts += 1
				if attempts % attemptsPerMessage == 0:
					print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ": " + word1.strip().upper() + ", " + end)
